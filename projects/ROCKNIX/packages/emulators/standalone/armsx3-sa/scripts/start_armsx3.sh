...
# Keep ARMSX3 settings and JIT/shader caches separate from RPCS3 while sharing
# the PS3 firmware and virtual HDD data below.
...
# Share firmware, installed PSN content, saves and trophy data with RPCS3.
# Per-game configs and caches intentionally stay separate for clean A/B testing.
...
CONFIG_YML="${ARMSX3_CONFIG_ROOT}/config.yml"
...
if [[ "${1}" == *.psn ]]; then
  ...
  read -r PSNID < "${1}"
  GAME_PATH="${ARMSX3_CONFIG_ROOT}/dev_hdd0/game/${PSNID}/USRDIR/EBOOT.BIN"
elif [[ "${1}" == *.m3u ]]; then
...
if [ "$SUI" = "true" ]; then
  ...
  ${EMUPERF} /usr/bin/armsx3-sa
else
  export QT_QPA_PLATFORM=xcb
  set_kill set "-9 armsx3-sa"
  ${EMUPERF} /usr/bin/armsx3-sa --no-gui "$GAME_PATH"
fi
