...
steam_arm64_binfmt_and_proton_prep() {
  echo 1 >/proc/sys/fs/binfmt_misc/x86_64 2>/dev/null || true
  echo 1 >/proc/sys/fs/binfmt_misc/x86 2>/dev/null || true
  local proton_dir="/storage/.local/share/Steam/steamapps/common/Proton 11.0 (ARM64)"
  if [ ! -L "$proton_dir" ]; then
    rm -rf "$proton_dir"
    local cachy_dir
    cachy_dir=$(ls -d /storage/.local/share/Steam/compatibilitytools.d/proton-cachyos-* 2>/dev/null | head -n 1)
    if [ -n "$cachy_dir" ]; then
      ln -sfn "$cachy_dir" "$proton_dir"
    else
      mkdir -p "$proton_dir"
    fi
  fi
